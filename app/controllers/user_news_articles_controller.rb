class UserNewsArticlesController < ApplicationController
  def index
    @user_articles = policy_scope(UserNewsArticle).all
  ...
  def scrape_bbc
    if params[:search].present?
      ...
      html_doc.search('.ssrcss-rn9nnc-PromoSwitchLayoutAtBreakpoints.et5qctl0').each do |element|
        ...
        begin
          bbc_user_articles << UserNewsArticle.create!(source: 'BBC', title: bbc_title, content: bbc_content, image: bbc_image, link: bbc_link, published: bbc_published, user: current_user)
        rescue ActiveRecord::RecordInvalid => e
          puts "Error creating UserNewsArticle: #{e.message}"
        end
      end
      @sorted_bbc_user_articles = sort_bbc(bbc_user_articles)
    end
    @sorted_bbc_user_articles
  end

  def sort_bbc(bbc_user_articles)
    new_bbc_articles = []
    bbc_hours = UserNewsArticle.where('user_id = ? AND source = ? AND published LIKE ?', current_user.id.to_s, 'BBC', '%hour%')
    bbc_days = UserNewsArticle.where('user_id = ? AND source = ? AND published LIKE ?', current_user.id.to_s, 'BBC', '%day%')
    bbc_this_year = UserNewsArticle.where('user_id = ? AND source = ? AND published LIKE ?', current_user.id.to_s, 'BBC', "%#{Date.today.year}%")
    bbc_user_articles -= bbc_hours
    bbc_user_articles -= bbc_days
    bbc_user_articles -= bbc_this_year
    new_bbc_articles << bbc_hours
    new_bbc_articles << bbc_days
    new_bbc_articles << bbc_this_year
    new_bbc_articles << bbc_user_articles
    new_bbc_articles.flatten
  end

  def scrape_politico
    if params[:search].present?
      @politico_user_articles = []
      ...
      html_doc.search('.story-frag.format-ml').each do |element|
        ...
        begin
          @politico_user_articles << UserNewsArticle.create!(source: 'Politico', title: politico_title, content: politico_content, image: politico_image, link: politico_link, published: politico_published, user: current_user)
        rescue ActiveRecord::RecordInvalid => e
          puts "Error creating UserNewsArticle: #{e.message}"
        end
      end
  ...
  #     #   @cnn_articles << UserNewsArticle.create!(source: 'CNN')
  #     # end
  #   end
  #   @cnn_articles
  # end
  ...
  #       @reuters_articles << UserNewsArticle.create!(source: 'REUTERS', title: @reuters_title, content: @reuters_content, image: @reuters_image, link: @reuters_link, published: @reuters_published, user: current_user)
  #     end
  #   end
  #   @reuters_articles
  # end
  ...
  def scrape_nyt
    if params[:search].present?
      ...
      html_doc.search('.css-1i8vfl5').each do |element|
        ...
        begin
          @nyt_user_articles << UserNewsArticle.create!(source: 'NYT', title: nyt_title, content: nyt_content, image: nyt_image, link: nyt_link, published: nyt_published, user: current_user)
        rescue ActiveRecord::RecordInvalid => e
          puts "Error creating UserNewsArticle: #{e.message}"
        end
      end
  ...
  #       @bloomberg_articles << UserNewsArticle.create!(source: 'bloomberg', title: @bloomberg_title, content: @bloomberg_content, image: @bloomberg_image, link: @bloomberg_link, published: @bloomberg_published, user: current_user)
  #     end
  #   end
  #   @bloomberg_articles
  #   raise
  ...
  def scrape_al_jazeera
    if params[:search].present?
      ...
      html_doc.search('article').each do |element|
        ...
        begin
          @al_jazeera_user_articles << UserNewsArticle.create!(source: 'Al Jazeera', title: al_jazeera_title, content: al_jazeera_content, image: al_jazeera_image, link: al_jazeera_link, published: al_jazeera_published, user: current_user)
        rescue ActiveRecord::RecordInvalid => e
          puts "Error creating UserNewsArticle: #{e.message}"
        end
      end
  ...
  def scrape_japan_times
    if params[:search].present?
      ...
      html_doc.search('.article').first(20).each do |element|
        ...
        begin
          @japan_times_user_articles << UserNewsArticle.create!(source: 'Japan Times', title: japan_times_title, content: japan_times_content, image: japan_times_image, link: japan_times_link, published: japan_times_published, user: current_user)
        rescue ActiveRecord::RecordInvalid => e
          puts "Error creating UserNewsArticle: #{e.message}"
        end
      end
  ...
  def scrape_ap
    if params[:search].present?
      ...
      html_doc.search('.PagePromo').each do |element|
        ...
        begin
          @ap_user_articles << UserNewsArticle.create!(source: 'AP', title: ap_title, content: ap_content, image: ap_image, link: ap_link, published: ap_published, user: current_user)
        rescue ActiveRecord::RecordInvalid => e
          puts "Error creating UserNewsArticle: #{e.message}"
        end
      end
